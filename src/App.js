...
function App() {
  ...
  useEffect(() => {
    ...
    const MAP = {
      ...
      areas: [
        {"shape": "circle", "coords": [105, 80, 8], "preFillColor": nodeData["node1"]?.status == "on fire" ? "#d84c2c" : "green"},
        {"shape": "circle", "coords": [185, 50, 8], "preFillColor": nodeData["node2"]?.status == "on fire" ? "#d84c2c" : "green"},
        {"shape": "circle", "coords": [273, 25, 8], "preFillColor": nodeData["node3"]?.status == "on fire" ? "#d84c2c" : "green"}],
    };
      ...
      <table id="rcorners1">
        ...
        <tbody>
          {Object.keys(nodeData).map((key) => {
            return (
              <tr className={nodeData[key]?.status == "on fire" ? "fire" : nodeData[key]?.status == "not on fire" ? "no-fire" : "pass"}>
                <td>{key}</td>
